...
# Define UI for application that plays the card game 24
ui <- fluidPage(
   ...
   sidebarLayout(
      sidebarPanel(
        actionButton("button", "Show me how to get 24!"),
        actionButton("reset", "Play again")
      ),
      
      # Show cards then solutions
      mainPanel(
         textOutput("fieldCards"),
         tableOutput("solutions"),
         tags$head(tags$style("#fieldCards{color: blue;    
                                font-size: 40px;
                                font-style: bold;
...
server <- function(input, output) {
  ...
  # This function gives 4 random cards and prompts the user before giving the solutions to 24
  game24_makeCardsShiny=function(A = 0, b=24){
    if(A == 0){
    A = sample(1:13, 4, replace = TRUE)
    }
    output$fieldCards = renderText({
      paste(c("Make 24 from these cards:", A), collapse = " ")
      })
    len=length(A)
    B=combinat::permn(A)
    ...
    if(length(result1) == 0){
      output$solutions <- eventReactive(input$button, 
      {paste("You cannot make 24 with these cards.")})
      return(0)
    }
    ...
    output$solutions <- renderTable({df()})
    
  }
  
  game24_makeCardsShiny()
  #eventReactive(input$reset, {game24_makeCardsShiny()})
